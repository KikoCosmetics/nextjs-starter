import {KikAsyncValidatorFn as AsyncValidatorFn, KikValidatorFn as ValidatorFn} from "../models/kik-validator.model";
import {KikAbstractControl as AbstractControl} from "../models/kik-abstract-control.class";
import {KikAbstractControlOptions as AbstractControlOptions} from "../models/kik-abstract-control-options.interface";
import {KikFormArray as FormArray, UntypedFormArray} from "../models/kik-form-array.class";
import {
    KikFormControlOptions as FormControlOptions,
    UntypedFormControl,
    KikFormControlState as FormControlState,
    KikFormControl
} from "../models/kik-form-control.class";
import {
    KikFormGroup as FormGroup,
    KikFormRecord as FormRecord,
    UntypedFormGroup
} from "../models/kik-form-group.class";
...
/**
 * KikFormBuilder accepts values in various container shapes, as well as raw values.
 * Element returns the appropriate corresponding model class, given the container T.
 * The flag N, if not never, makes the resulting `KikFormControl` have N in its type.
 */
...
//
// Identify KikFormControl container types.
    [T] extends [KikFormControl<infer U>] ? KikFormControl<U> :
        // Or KikFormControl containers that are optional in their parent group.
        [T] extends [KikFormControl<infer U> | undefined] ? KikFormControl<U> :
                                ...
                                [T] extends [FormArray<infer U> | undefined] ? FormArray<U> :
                                    ...
                                    [T] extends [AbstractControl<infer U>] ? AbstractControl<U> :
                                        ...
                                        [T] extends [AbstractControl<infer U> | undefined] ? AbstractControl<U> :
                                            ...
                                            [T] extends [FormControlState<infer U>] ? KikFormControl<U | N> :
                                                // A ControlConfig tuple, which produces a nullable control.
                                                [T] extends [PermissiveControlConfig<infer U>] ? KikFormControl<Exclude<U, ValidatorConfig | PermissiveAbstractControlOptions> | N> :
                                                    KikFormControl<T | N>;
 ...
 *
 * The `KikFormBuilder` provides syntactic sugar that shortens creating instances of a
 * `KikFormControl`, `FormGroup`, or `FormArray`. It reduces the amount of boilerplate needed to
 * build complex forms.
 *
...
export class KikFormBuilder {

    /**
     * @description
     * Returns a KikFormBuilder in which automatically constructed `KikFormControl` elements
     ...
     *
     * ```ts
     * let nnfb = new KikFormBuilder().nonNullable;
     * let name = nnfb.control('Alex'); // KikFormControl<string>
     * name.reset();
     ...
     * ```ts
     * let nnfb = new KikFormBuilder().nonNullable;
     * let name = nnfb.group({who: 'Alex'}); // FormGroup<{who: KikFormControl<string>}>
     * name.reset();
     * console.log(name); // {who: 'Alex'}
     * ```
     * **Constructing *nullable* fields on groups or arrays**
     *
     * It is still possible to have a nullable field. In particular, any `KikFormControl` which is
     * *already* constructed will not be altered. For example:
     *
     * ```ts
     * let nnfb = new KikFormBuilder().nonNullable;
     * // FormGroup<{who: KikFormControl<string|null>}>
     * let name = nnfb.group({who: new KikFormControl("Alex")});
    ...
    constructor() {
        console.info("FORM BUILDER CONSTRUCTED")
    }

    /** @internal */
    // eslint-disable-next-line @typescript-eslint/naming-convention
    _createControl<T>(controls: T | FormControlState<T> | ControlConfig<T> | KikFormControl<T> |
        AbstractControl<T>): KikFormControl<T> | KikFormControl<T | null> | AbstractControl<T> {
        if (controls instanceof KikFormControl) {
            return controls as KikFormControl<T>;
        }
    ...
    /**
     * @description
     * Constructs a new `KikFormControl` with the given state, validators and options. Sets
     ...
     * such functions, or a `KikFormControlOptions` object that contains
     * validation functions and a validation trigger.
     *
    ...
    control<T>(
        formState: T | FormControlState<T>,
        validatorOrOpts?: ValidatorFn | ValidatorFn[] | FormControlOptions | null,
        asyncValidator?: AsyncValidatorFn | AsyncValidatorFn[] | null): KikFormControl {
        let newOptions: FormControlOptions = {};
        if (!this.useNonNullable) {
            return new KikFormControl(formState, validatorOrOpts, asyncValidator);
        }
        ...
        return new KikFormControl<T>(formState, {
            ...newOptions,
            nonNullable: true
        });
    }
     ...
     * Use the `KikFormBuilder#group` overload with `AbstractControlOptions` instead.
    ...
    group(controls: { [key: string]: any }, options: AbstractControlOptions | {
        [key: string]:
            any
    } | null = null):
        FormGroup {
...
/**
 * @description
 * `NonNullableFormBuilder` is similar to {@link KikFormBuilder}, but automatically constructed
 * {@link KikFormControl} elements have `{nonNullable: true}` and are non-nullable.
 *
 * @publicApi
 */
// @Injectable({
//                 providedIn: "root",
//                 useFactory: () => inject(KikFormBuilder).nonNullable
//             })
export abstract class NonNullableFormBuilder {
    /**
     * Similar to `KikFormBuilder#group`, except any implicitly constructed `KikFormControl`
    ...
    /**
     * Similar to `KikFormBuilder#record`, except any implicitly constructed `KikFormControl`
     * will be non-nullable (i.e. it will have `nonNullable` set to true). Note
     * that already-constructed controls will not be altered.
     */
    abstract record<T>(
    ...
    /**
     * Similar to `KikFormBuilder#array`, except any implicitly constructed `KikFormControl`
     * will be non-nullable (i.e. it will have `nonNullable` set to true). Note
     * that already-constructed controls will not be altered.
     */
    abstract array<T>(
    ...
    /**
     * Similar to `KikFormBuilder#control`, except this overridden version of `control` forces
     * `nonNullable` to be `true`, resulting in the control always being non-nullable.
     */
    abstract control<T>(
        formState: T | FormControlState<T>,
        validatorOrOpts?: ValidatorFn | ValidatorFn[] | AbstractControlOptions | null,
        asyncValidator?: AsyncValidatorFn | AsyncValidatorFn[] | null): KikFormControl<T>;
}

/**
 * UntypedFormBuilder is the same as `KikFormBuilder`, but it provides untyped controls.
 */
// @Injectable({providedIn: "root"})
export class UntypedFormBuilder extends KikFormBuilder {

    /**
     * Like `KikFormBuilder#array`, except the resulting array is untyped.
     */
    override array(
        controlsConfig: any[],
        validatorOrOpts?: ValidatorFn | ValidatorFn[] | AbstractControlOptions | null,
    ...
    /**
     * Like `KikFormBuilder#control`, except the resulting control is untyped.
     */
    override control(
    ...
    /**
     * Like `KikFormBuilder#group`, except the resulting group is untyped.
     */
    override group(
